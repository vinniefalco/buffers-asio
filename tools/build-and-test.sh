...
function valgrind_run ()
{
  valgrind \
    ...
    --suppressions=$BOOST_ROOT/libs/$LIB_NAME/tools/valgrind.supp \
    --error-exitcode=1 \
    $@
}
...
BIN_DIR="$BOOST_ROOT/bin.v2/libs/$LIB_NAME/test"
LIB_DIR="$BOOST_ROOT/libs/$LIB_NAME"
INC_DIR="$BOOST_ROOT/boost/$LIB_NAME"
...
if [[ $VARIANT == "coverage" ]]; then
  ...
  run_tests_with_valgrind "$BIN_DIR" run-tests

else
  run_tests_with_debugger "$BIN_DIR" run-tests
